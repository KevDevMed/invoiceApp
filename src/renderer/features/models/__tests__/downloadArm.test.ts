...
describe('downloadArm', () => {
  it('loads once the transfer it armed reports ready', () => {
    ...
    const attached = attachModelId(armed.state, armed.token, ID);
  ...
  it('loads when the terminal event beat the invoke it was armed for', () => {
    ...
    const attached = attachModelId(early.state, armed.token, ID);
  ...
  it('does not load when the event that beat the invoke was not a success', () => {
    ...
    expect(attachModelId(early.state, armed.token, ID).shouldLoad).toBe(false);
  });
  ...
  it('ignores a terminal event that predates the arm', () => {
    ...
    expect(attachModelId(armed.state, armed.token, ID).shouldLoad).toBe(false);
  });
  ...
  it('revokes an arm that never got its model id when the file is plainly downloaded', () => {
    ...
    expect(attachModelId(plain, armed.token, ID).shouldLoad).toBe(false);
    expect(noteTerminal(attachModelId(plain, armed.token, ID).state, ID, 'ready', 2_000).shouldLoad).toBe(
      false,
    );
  });
  ...
  it('arms nothing when the invoke threw', () => {
    ...
    expect(attachModelId(cancelled, armed.token, ID).shouldLoad).toBe(false);
  ...
  it('supersedes an earlier arm on the same file rather than loading twice', () => {
    ...
    expect(attachModelId(second.state, first.token, ID).shouldLoad).toBe(false);

    const attached = attachModelId(second.state, second.token, ID);
